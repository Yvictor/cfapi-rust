...
impl From<RedisSinkError> for SinkError {
    fn from(err: RedisSinkError) -> Self {
        match err {
            RedisSinkError::Connection { source } => SinkError::RedisConnection { source },
            RedisSinkError::Command { source } => SinkError::RedisCommand { source },
        }
    }
}
...
impl Default for RedisSink {
    fn default() -> Self {
        ...
        let key_prefix = dotenvy::var("REDIS_KEY_PREFIX").unwrap_or_else(|_| "cfvhub".to_string());
        let data_structure =
            dotenvy::var("REDIS_DATA_STRUCTURE").unwrap_or_else(|_| "list".to_string());
...
impl RedisSink {
    ...
    fn push_to_stream(&self, key: &str, data: &str) -> Result<(), RedisSinkError> {
        let mut conn = self.connection.lock().unwrap();
        conn.xadd::<_, _, _, _, ()>(key, "*", &[("data", data)])
        ...
        match formated {
            Ok(formated) => {
                let data = match formated {
                    Formated::String(s) => s,
                    Formated::Bytes(b) => match String::from_utf8(b) {
                        Ok(s) => s,
                        Err(e) => {
